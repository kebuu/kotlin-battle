...
class SpawnTest {
    ...
    @Test
    fun validateUpdate() {
        assertThat(spawn.validateUpdate(SpawnAttributes(4), 2).isOk()).isFalse()
        assertThat(spawn.validateUpdate(SpawnAttributes(6), 2).isOk()).isFalse()
        assertThat(spawn.validateUpdate(SpawnAttributes(-1), 2).isOk()).isFalse()
        assertThat(spawn.validateUpdate(SpawnAttributes(6), 4).isOk()).isTrue()
    }
    ...
    fun updateRandomly() {
        for (i in 0..20) {
            ...
            assertThat(spawn.validateUpdate(newSpawnAttributes, point).isOk()).isTrue()
        }
    }
